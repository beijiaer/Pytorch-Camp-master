...
class RMBDataset(Dataset):
    def __init__(self, data_dir, transform=None):
        ...
        self.data_info = self.get_img_info(data_dir)  # data_info存储所有图片数据(不是路径)和标签，在DataLoader中通过index读取样本
    ...
    def get_img_info(data_dir):
        ...
        for root, dirs, _ in os.walk(data_dir):
            # 遍历类别
            for sub_dir in dirs:
                img_names = os.listdir(os.path.join(root, sub_dir))#返回root/sub_dir文件夹下的所有文件
                img_names = list(filter(lambda x: x.endswith('.jpg'), img_names))#过滤，得到所有以.jpg结尾的文件
